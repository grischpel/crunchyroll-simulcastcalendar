(() => {
  ...
  const applySavedUserColor = (value) => {
    ...
    document.documentElement.style.setProperty(
      "--crunchyroll-simulcast-progressbar-background-color",
      color,
    );
  };

  chrome.storage.local.get(["userColor"], (result) => {
    applySavedUserColor(result.userColor);
  });

  chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === "local" && changes.userColor) {
      applySavedUserColor(changes.userColor.newValue);
    }
  });

  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message?.type === "updateUserColor" && typeof message.color === "string") {
      applySavedUserColor(message.color);
    }
    return false;
  });
  ...
  const createResetButton = () => {
    ...
    btn.addEventListener("click", () => {
      ...
      const mainFilterAll = document.querySelector(
  ...
  const createLanguageCheckbox = (languageCode) => {
    ...
    checkbox.addEventListener("change", () => {
      ...
      applyQueueFilter(
        document.querySelector(".in-queue-checkbox")?.checked === true,
      );
      applyAlternatingRowStyles();
    });
  ...
  const renderLanguageFilterContainer = () => {
    const langFilterContainer = document.querySelector(
      "#language_filter_container",
    );
    ...
    if (sortedLanguageCodes.length > 0) {
      ...
      const savedLangs = JSON.parse(
        localStorage.getItem("languageFilter") || "[]",
      );
      if (savedLangs.length > 0) {
        const checkboxes =
          langFilterContainer.querySelectorAll(".language-checkbox");
